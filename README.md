...
* `zephir_upd_YYYYMMDD_delete.txt.gz`  will be moved to /htsolr/catalog/prep 
* `hathi_upd_YYYYMMDD.txt` hathifile, moved to /htapps/www/sites/www.hathitrust.org/files/hathifiles
...
* `meta_(ic|pd_google|pd_open_access|restricted)_20200107.jsonl.gz` (currently going to `/aleph-22_1/aleph/uprod/miu50/local/mdp_batch/return/transfer`) Used by datasets. Currently copied from gimlet to `/htprep/datasets/ht_bib` at ICTC by `/l/local/bin/getmeta.sh` on quik-1. New process will put into `/htdata/return/transfer`
