...
my $fixtures_dir = dirname(__FILE__) . '/fixtures/bib_rights/';
...
subtest "Miscellaneous coverage tests" => sub {
  ...
  ok(my $br2 = create_bib_rights($br));
...
END_OF_RECORD
  my $fake_marc = MARC::Record->new_from_xml($fake_marc_xml);
  my $br = create_bib_rights();
  my $bib_info = bib_info($br, $fake_marc, 'BOGUS RECORD 1');
...
END_OF_RECORD
  my $fake_marc = MARC::Record->new_from_xml($fake_marc_xml);
  my $br = create_bib_rights();
  my $bib_info = bib_info($br, $fake_marc, 'BOGUS RECORD 4');
...
END_OF_RECORD
  my $br = create_bib_rights();
  my $fake_marc = MARC::Record->new_from_xml($fake_marc_xml);
  my $bib_info = bib_info($br, $fake_marc, 'BOGUS RECORD 6');
...
END_OF_RECORD
  my $br = create_bib_rights();
  my $fake_marc = MARC::Record->new_from_xml($fake_marc_xml);
  my $fmt = bib_fmt('BOGUS RECORD 5', $fake_marc);
  is($fmt, 'SE');
};
...
sub run_tests_for_htid {
  ...
  foreach my $year (sort keys %{$test_struct->{$htid}->{years}}) {
    ...
    my $br = create_bib_rights();
    my $bib_info = bib_info($br, $marc, $cid);
    my $bib_rights_info = $br->get_bib_rights_info($htid, $bib_info, $enumcron);
    ok($bib_rights_info->{attr} eq $attr, "$htid: $year $bib_rights_info->{attr} vs $attr");
...
sub create_bib_rights {
  my $existing_br = shift; # Defined if cloning
  my $br;

  my ($stderr, @result) = Capture::Tiny::capture_stderr sub {
    $br = (defined $existing_br) ? $existing_br->new : bib_rights->new;
  };
  return $br;
}

# Ditto the above, capture and ignore chatter.
sub bib_info {
  my $bib_rights = shift;
  my $marc       = shift;
  my $cid        = shift;
  my $bi;
  my ($stderr, @result) = Capture::Tiny::capture_stderr sub {
    $bi = $bib_rights->get_bib_info($marc, $cid);
  };
  return $bi;
}
...
sub bib_fmt {
  ...
  my $fmt;
  my ($stderr, @result) = Capture::Tiny::capture_stderr sub {
    $fmt = bib_rights::getBibFmt($cid, $marc);
  };
  return $fmt;
}
